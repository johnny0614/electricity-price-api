...
describe('AuthService', () => {
  ...
  describe('environment variable handling', () => {
    it('should work with legacy API_USERNAME and API_PASSWORD format', () => {
      ...
      expect(
        legacyAuthService.validateCredentials('legacyuser', 'legacypass')
      ).resolves.toBe(true);
    });
    ...
    it('should prioritize API_USERS over legacy format', () => {
      ...
      expect(
        authService.validateCredentials('newuser', 'newpass')
      ).resolves.toBe(true);
      expect(
        authService.validateCredentials('legacyuser', 'legacypass')
      ).resolves.toBe(false);
    });
    ...
    it('should handle malformed API_USERS format gracefully', () => {
      ...
      const authService = new AuthService();
      expect(authService.validateCredentials('user3', 'pass3')).resolves.toBe(
        true
      );
      expect(
        authService.validateCredentials('invalidformat', '')
      ).resolves.toBe(false);
    });
  });
});
